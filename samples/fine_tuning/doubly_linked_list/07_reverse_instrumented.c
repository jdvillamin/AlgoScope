...
int main() {
    ...
    while (curr != NULL) {
        ...
        trace_line(46);
        Node* temp = curr->next;
        trace_dll_pointer("dll", "temp", temp ? temp->id : "NULL");
        trace_line(47);
        trace_dll_next_unlink("dll", curr->id, temp ? temp->id : "NULL");
        curr->next = curr->prev;
        if (curr->prev) trace_dll_next_link("dll", curr->id, curr->prev->id);
        trace_line(48);
        trace_dll_prev_unlink("dll", curr->id, curr->next ? curr->next->id : "NULL");
        curr->prev = temp;
        if (temp) trace_dll_prev_link("dll", curr->id, temp->id);
        trace_line(49);
        curr = temp;
    ...
    trace_line(54);
    tail = swap;
    trace_dll_pointer("dll", "tail", tail->id);
