...
int main() {
    ...
    trace_line(44);
    Node* temp = tail;
    trace_dll_pointer("dll", "temp", tail->id);
    trace_line(45);
    trace_dll_prev_unlink("dll", tail->id, tail->prev->id);
    tail = tail->prev;
    trace_dll_pointer("dll", "tail", tail->id);
    trace_line(46);
    trace_dll_next_unlink("dll", tail->id, temp->id);
    tail->next = NULL;
